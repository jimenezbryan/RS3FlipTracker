...
function loadStorage(): { checked: CheckedMap } {
  try {
...
function saveStorage(data: { checked: CheckedMap }) {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(data))
  } catch {}
}

function getAccountProgress(account: Account, checked: CheckedMap) {
  let total = 0, done = 0
...
function getSectionProgress(section: Section, checked: CheckedMap) {
...
function ItemTag({ type }: { type: string }) {
  ...
  const cls = type === 'priority'
    ? 'border border-[#8a3a2a] bg-[#8a3a2a]/20 text-[#ff7a5a]'
    : 'border border-[var(--arch-gold-dim)] bg-[var(--arch-gold)]/10 text-[var(--arch-gold-light)]'
  return (
    <span className={`inline-block px-1.5 py-0 rounded-sm text-[9px] tracking-widest ml-1.5 align-middle font-cinzel-arch ${cls}`}>
      {label}
    </span>
  )
}

function CheckItem({ item, checked, onToggle }: { item: Item; checked: boolean; onToggle: (id: string) => void }) {
  return (
    <div
      role="checkbox"
      ...
      className={`flex items-start gap-3 px-4 py-2.5 border-b border-[var(--arch-bg3)] last:border-b-0 cursor-pointer select-none transition-colors duration-150 hover:bg-[var(--arch-gold)]/[0.03] ${checked ? 'opacity-50' : ''}`}
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm leading-snug text-[var(--arch-text-bright)] font-crimson-arch ${checked ? 'line-through decoration-[var(--arch-gold-dim)]' : ''}`}>
          {item.text}
          {item.tags.map(t => <ItemTag key={t} type={t} />)}
        </p>
        {item.note && (
          <p className="text-xs text-[var(--arch-text-dim)] italic mt-0.5 font-crimson-arch font-light">
            {item.note}
          </p>
        )}
      </div>
...
function TrackerSection({ section, checked, onToggle, defaultOpen }: {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <div className={`rounded-sm border transition-colors duration-200 overflow-hidden ${allDone ? 'border-[var(--arch-green)] bg-[var(--arch-green)]/5' : 'border-[var(--arch-border)] hover:border-[var(--arch-border-bright)]'}`}>
        <CollapsibleTrigger asChild>
          <div
            data-testid={`section-trigger-${section.id}`}
            className={`flex items-center gap-3 px-4 py-3 cursor-pointer select-none transition-colors duration-150 ${allDone ? 'bg-[var(--arch-green)]/8' : 'bg-[var(--arch-panel2)] hover:bg-[#2a2010]'}`}
          >
            <span className="text-base shrink-0">{allDone ? '✅' : section.icon}</span>
            <span className={`font-cinzel-arch text-[10px] tracking-[0.15em] flex-1 ${allDone ? 'text-[var(--arch-green-bright)]' : 'text-[var(--arch-gold)]'}`}>
              {section.title}
            </span>
            <span className="text-[11px] text-[var(--arch-text-dim)] italic font-crimson-arch font-light hidden sm:block">
              {section.meta}
            </span>
            <span className={`font-cinzel-arch text-[10px] ml-2 shrink-0 ${allDone ? 'text-[var(--arch-green-bright)]' : 'text-[var(--arch-text-dim)]'}`}>
              {done}/{total}
            </span>
            {open
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3 bg-[var(--arch-panel)] border border-[var(--arch-border)] rounded-sm px-4 py-3">
        <span className="font-cinzel-arch text-[9px] tracking-[0.18em] text-[var(--arch-text-dim)] shrink-0">
          OVERALL
        </span>
        <Progress
          value={pct}
          className="flex-1 h-1.5 bg-[var(--arch-bg3)] [&>div]:bg-gradient-to-r [&>div]:from-[var(--arch-gold-dim)] [&>div]:to-[var(--arch-gold-light)]"
          data-testid={`progress-${account.id}`}
        />
        <span className="font-cinzel-arch text-xs text-[var(--arch-gold-light)] shrink-0">
      ...
      <div className="pt-2 pb-1 text-center">
        <button
          ...
          onClick={onReset}
          className="font-cinzel-arch text-[9px] tracking-[0.18em] text-[#ff7a5a] border border-[#8a3a2a] px-4 py-1.5 rounded-sm hover:bg-[#8a3a2a]/20 transition-colors duration-150"
        >
...
export default function Archaeology() {
  ...
  const resetAccount = useCallback((account: Account) => {
  ...
  return (
    <>
      <style>{`
        ...
        .font-cinzel-arch { font-family: 'Cinzel', serif; }
        .font-crimson-arch { font-family: 'Crimson Pro', Georgia, serif; }
      ...
      <div
        className="arch-tracker min-h-full bg-[var(--arch-bg)] text-[var(--arch-text)] font-crimson-arch"
        style={{
          backgroundImage: `
      ...
      >
        <div className="max-w-3xl mx-auto px-4 py-8">
          <header className="text-center pb-7 mb-7 border-b border-[var(--arch-border)]">
            <p className="font-cinzel-arch text-[9px] tracking-[0.3em] text-[var(--arch-gold-dim)] mb-3">
              — ✦ —
            </p>
            <h1
              className="font-cinzel-arch font-bold text-2xl sm:text-3xl tracking-[0.2em] text-[var(--arch-gold-light)]"
            ...
            <p className="font-cinzel-arch text-[9px] tracking-[0.3em] text-[var(--arch-gold-dim)] mt-3">
              — ✦ —
            </p>
          </header>
              ...
              {ACCOUNTS.map(acc => {
                ...
                return (
                  <TabsTrigger
                    key={acc.id}
                    value={acc.id}
                    data-testid={`tab-${acc.id}`}
                    className="flex-1 flex flex-col py-2.5 px-3 rounded-sm h-auto data-[state=active]:bg-[var(--arch-panel2)] data-[state=active]:text-[var(--arch-gold-light)] data-[state=active]:border-b-2 data-[state=active]:border-[var(--arch-gold)] data-[state=inactive]:text-[var(--arch-text-dim)] transition-colors duration-150"
                  >
                    <span className="font-cinzel-arch text-[10px] tracking-[0.18em]">{acc.name.toUpperCase()}</span>
                    <span className="font-crimson-arch font-light text-[10px] italic mt-0.5 opacity-70">
